			...
			for (tokenizer in tokenizers) {
				...
				if (tokenizer.IsMatch(peek, reader, variableDefinitions, functionDefinitions, scope.first(), tokens)) {
					...
					if (tokenizer is FunctionDefinitionTokenizer) {
						scope.add(tokens.last().tokenBuffer);
					}
					if (tokenizer is ForLoopTokenizer) {
						scope.add(scope.first() + "_" + tokens.last().tokenBuffer);
					}
					if (tokenizer is WhileLoopTokenizer) {
						scope.add(scope.first() + "_" + tokens.last().tokenBuffer);
					}
					if (tokenizer is SingleCharTokenizer) {
						var singleCharTokenizer:SingleCharTokenizer = cast tokenizer;
						if (singleCharTokenizer.tokenType == TokenType.RightScope) {
							scope.pop();
						}
					}
					succeeded = true;
