...
abstract class Tokenizer {
    public abstract function GetToken(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String): Token;
    public abstract function IsMatch(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String, existingTokens: List<Token>): Bool;
}
...
class SingleCharTokenizer extends Tokenizer {
    ...
    public function GetToken(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String): Token {
    ...
    public function IsMatch(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String, existingTokens: List<Token>): Bool {
...
class AssignmentTokenizer extends Tokenizer {
    ...
    public function IsMatch(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String, existingTokens: List<Token>): Bool {
        if (!Utility.searchForPotential(peekChar, variableDefintions)) return false;
...
class VariableTokenizer extends Tokenizer {
    ...
    public function IsMatch(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String, existingTokens: List<Token>): Bool {
        ...
        if (!Utility.searchForPotential(peekChar, variableDefintions) && !Utility.searchForPotentialArray(peekChar, args)) return false;

        var stringBuf: StringBuf = new StringBuf();
        var check: Int = 0;
        while (
            !Lambda.exists(variableDefintions, function(x: String) { return x == stringBuf.toString(); }) && 
            !Lambda.exists(args, function(x: String) { return x == stringBuf.toString(); }) &&
            check < 25) {
        ...
        return Lambda.exists(variableDefintions, function(x: String) { return x == stringBuf.toString(); }) || Lambda.exists(args, function(x: String) { return x == stringBuf.toString(); });
    }
}
...
class FunctionTokenizer extends Tokenizer {
    ...
    public function GetToken(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String): Token {
        var functionNames = functionDefinitions.map(function(x: FunctionDefinition) { return x.name; });
        var stringBuf: StringBuf = new StringBuf();
        while (!Lambda.exists(functionNames, function(x: String) { return x == stringBuf.toString(); }) && reader.Peek() != "(") {
            stringBuf.add(reader.Read());
            if (reader._position >= reader._length) {
                break;
            }
    ...
    public function IsMatch(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String, existingTokens: List<Token>): Bool {
        var functionNames = functionDefinitions.map(function(x: FunctionDefinition) { return x.name; });
        if (!AscentFunctions.SearchAnyFunctions(peekChar) && !Utility.searchForPotential(peekChar, functionNames)) return false;

        var stringBuf: StringBuf = new StringBuf();
        while (AscentFunctions.GetFunction(stringBuf.toString()) == null && reader.Peek() != "(") {
            stringBuf.add(reader.Read());
            if (reader._position >= reader._length) {
                break;
            }
        ...
        return AscentFunctions.GetFunction(stringBuf.toString()) != null || Lambda.exists(functionNames, function(f: String) { return f == stringBuf.toString(); });
    }
}
...
class FunctionArgumentTokenizer extends Tokenizer {
    ...
    public function IsMatch(peekChar: Int, reader: StringReader, variableDefintions: List<String>, functionDefinitions: List<FunctionDefinition>, scope: String, existingTokens: List<Token>): Bool {
        ...
        if (toks == null || toks.length < back + 1) return false;
