...
import parser.Expression.TernaryCondition;
...
class AscentParser {
    ...
    private function LoadNextContainer()
    {
        while (_position >= _currentTokens.length && _containerStack.length > 0)
        {
            ...
            else if (currentContainer is MultipleTokenContainer)
            {
                ...
                {
                    _containerStack.insert(0, multiple.tokenContainers[i]);
                }
            }
        }
    }

    public function Parse(variableMap: AscentVariableMap): List<Expression> {
    ...
    private function ParseBinary(precedence: Int, variableMap: AscentVariableMap): Expression {
        ...
        while (true)
        {
            if (_position >= _currentTokens.length || !Utility.searchMapKeysForTokenType(_currentTokens[_position].type, Precedence.keys()))
            {
                break;
            }
            ...
            if (operatorToken.type == TokenType.TernaryConditional)
            {
                ...
                return new TernaryCondition(condition, trueExpression, falseExpression);
            }
            else
            {
