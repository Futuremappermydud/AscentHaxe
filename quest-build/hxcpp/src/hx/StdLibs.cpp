...
void __trace(Dynamic inObj, Dynamic info)
{
   ...
   if (info==null())
   {
      __android_log_print(ANDROID_LOG_INFO, "Haxe", "?? %s\n", text.raw_ptr() ? text.out_str(&convertBuf) : "null");
   }
   else
   {
      ...
      __android_log_print(ANDROID_LOG_INFO, "Haxe", "%s:%d: %s\n", filename, line, text.raw_ptr() ? text.out_str(&convertBuf) : "null");
   }
...
{
   hx::strbuf convertBuf;
   __android_log_print(ANDROID_LOG_INFO, "Haxe", "%s", inV.out_str(&convertBuf));
}
